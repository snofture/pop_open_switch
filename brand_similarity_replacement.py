...
sc = SparkContext(appName='calculate brand vendor switch')
hc = HiveContext(sc)
...
# 获取品牌商会话点击信息
data = data.join(top_pop_id,'brand_code','inner')
data = data.withColumnRenamed('brand_code', 'from_brand_code').cache()
tmp = data.withColumnRenamed('from_brand_code', 'to_brand_code').withColumnRenamed('item_third_cate_id', 'to_cid3').withColumnRenamed('item_sku_id','to_sku_id').cache()
...
new_column2 = from_brand_total_switch.from_brand_code.cast("string")
from_brand_total_switch = from_brand_total_switch.withColumn('from_brand_code',new_column2)
